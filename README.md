...
- 后端：FastAPI + DuckDB（`excel` 扩展），通过 `read_xlsx` **直接读取原文件路径**，不做复制上传；xlsx 首查建 parquet 缓存加速。
- 前端：纯 HTML/CSS/JS 静态页面，点击**「选择文件」弹出 Windows 原生文件对话框**（或内置文件浏览器点选），字段点选、Ctrl+Enter 运行。
- **单文件 / 多源关联**两种模式：单文件用表名 `data`；多源为每个文件起别名（`t1`、`t2`…），可跨文件 SQL JOIN。
- **Notebook 式多 cell**：可写多条 SQL 各自运行，带大纲浮轨导航、虚拟滚动结果表、底部合计行。
- **最近打开**：记住历史文件与多源「组合」，一键恢复；文件被移动/删除自动标红提示。
- 全角/半角标点高亮（CodeMirror overlay），避免 SQL 逗号/括号混用不直观。
